...
def get_column_data(filename, column='DATA', startrow=0, nrow=-1):
    ms = table(filename)
    data = ms.getcol(column, startrow, nrow)
    return data

def get_ntimes(filename):
    ms = table(filename)
    return ms.nrows()
...
def read_adios_write_ms(inputfile, input_ms, column_name, accuracy, cyl=True, chunk_id=None, row_range=None):
    ms = table(input_ms, readonly=False)
    row0, row1 = row_range if row_range else (0, ms.nrows())
    nrows = row1 - row0
    if new_api:
        adios = adios2.Adios()
        io = adios.declare_io("ReadMgard")
        with adios2.Stream(io, inputfile, "r") as fr:
            for _ in fr.steps():
                if cyl:
                    ...
                    data = fr.read("real").astype(np.complex64)
                    data.imag = fr.read("imag")
                assert data.shape[0] == nrows, "Mismatch in row count for putcol"
                ms.putcol(column_name, data, startrow=row0, nrow=nrows)
    else:
        raise NotImplementedError("Old API not supported in write_ms")
...
def parse_args():
    ...
    parser.add_argument("-u", "--chunksize", type=int, help="Chunk size along the time axis")
...
if __name__ == '__main__':
    ...
    args = parse_args()
    ntimes = get_ntimes(args.input_ms)

    chunksize = args.chunksize
    if not chunksize:
        chunksize = ntimes
    accuracy_input = tuple(args.accuracy)
    logging.info(f"Ntimes={ntimes} Chunksize={chunksize}")

    for i, start in enumerate(range(0, ntimes, chunksize)):
        chunk_id = i
        row0 = start
        row1 = start + chunksize
        output_chunk = args.output_bp
        logging.info(f'Chunk {i}: {row0} - {row1}')
        if chunksize < ntimes:
            output_chunk = f"{args.output_bp}_chunk{chunk_id}"
        data = get_column_data(args.input_ms, args.column_name, startrow=row0, nrow=chunksize)
        nchans = data.shape[1]
        chuncksize = nchans
        mystart = 0
        myend = chuncksize
        ...
        logger.info(f"Writing data using adios")
        logger.info(f"chunk: {chunk_id}")
        logger.info(f"output: {output_chunk}")
        logger.info(f"operator: {args.operator}")
        logger.info(f"accuracy: {args.accuracy}")
        logger.info(f"mode: {args.mode}")
        logger.info(f"smoothness: {args.smoothness}")
        write_adios_high(data, output_chunk, args.operator, args.accuracy, args.mode, args.smoothness, args.cylindrical, args.stepwise)
        if args.numpy:
            read_adios_write_numpy(output_chunk, accuracy_input, args.column_name, args.cylindrical)
        if args.replace:
            output_ms = "_".join((os.path.basename(args.input_ms), args.column_name, args.accuracy[0], args.accuracy[1]))
            shutil.copytree(args.input_ms, output_ms, dirs_exist_ok=True)
            read_adios_write_ms(output_chunk, output_ms, args.column_name, args.accuracy, args.cylindrical, chunk_id=chunk_id, row_range=(row0, row0 + data.shape[0]))
